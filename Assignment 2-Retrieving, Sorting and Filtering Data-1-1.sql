...
-- SELECT CompanyName,COUNT(CompanyName)AS OrderCount

/*3.List the product name, unit price for each product. Sort the list with the largest unit price on top.*/
-- SELECT ProductName, UnitPrice FROM productes UnitPrice DESC;
...
-- SELECT ProductName FROM Products WHERE discuntined = 'y'  ;

/*6.List all products with the product name "Tofu". */
-- SELECT ProductName FROM Products WHERE ProductName = "Tofu"
 
/*7.List the product ID and product name for products with less than 100 units in stock and zero units on order. */
 -- SELECT ProductID, ProductName FROM Products WHERE UnitInStock < 100  AND UnitsOnOrder=0
...
-- SELECT ProductName, ProductID FROM Products WHERE SupplierID < 20

/*10.List the first name and last name of every employee. */
-- SELECT First name, last name   FROM Every employee

/*11.List the product name and units in stock for any product that has a units in stock greater then 10 and less then 50.*/
-- SELECT ProductName, UnitsInStock FROM Products WHERE UnitsInStock > 10 AND UnitInStock < 50
...
/*13.Show the order details for all of the orders that include product ID 20.*/
 -- SELECT * FROM order_details WHERE OrderID = 20 	
...
-- SELECT CompanyName, OrderDate FROM Customers, Orders WHERE Customers.CustomerID = Orders.CustomerID;
...
-- SELECT lastname, ShippedDate FROM Employees, Orders WHERE Employees.EmployeeID = Orders.EmployeeID
